...
export function useApi() {
  ...
  // ── dispatch a single WS message to matching listeners ─────────────
  ...
  const connect = useCallback((room) => {
    ...
    let url = `${WS_BASE}/ws?room=${encodeURIComponent(room)}`;
    if (lastVersionRef.current > 0) {
      url += `&lastVersion=${lastVersionRef.current}`;
    }
    ...
    ws.onmessage = (event) => {
      ...
      // Track version for delta replay on reconnect
      if (message._v != null) {
        lastVersionRef.current = message._v;
      }

      // Delta replay: server sends an array of missed broadcast messages
      if (message.type === 'delta_replay') {
        const messages = message.data || [];
        for (const msg of messages) {
          if (msg._v != null) lastVersionRef.current = msg._v;
          dispatchMessage(msg);
        }
        return;
      }
  ...
  const apiRef = useRef({
    ...
    // Player-Ready: signal that this player has completed their work for the current phase.
    // Server broadcasts a readiness summary to the room so NESO can see who is pending.
    playerSignalReady: (roomId, data) => api('POST', `/api/rooms/${roomId}/engine/player-ready`, data),
  });

...
/**
 * Apply NWP forecast refinement to a single SP's market.
 * Direct port of Python ida_forecast() — blends forecast toward actual by errorReduction.
 *
 * errorReduction values (matching _FORECAST_ERROR_REDUCTION in game_loop.py):
 *   FORECAST_0 → 1.0  (full uncertainty, no blend applied)
 *   FORECAST_1 → 0.6  (12Z run, ~40% error collapsed)
 *   FORECAST_2 → 0.3  (06Z short-range, ~70% error collapsed)
 *
 * @param {{ forecast: object, actual: object }} market  - result from marketForSp()
 * @param {number} errorReduction - fraction of remaining error to remove (0..1)
 * @returns {{ forecast: object, actual: object }} new market with refined forecast
 */
export function idaForecast(market, errorReduction) {
    const forecast = market?.forecast;
    const actual = market?.actual;
    if (!forecast || !actual) return market;

    const blend = (f, a) => (f == null || a == null) ? f : f + (a - f) * errorReduction;

    const refinedForecast = {
        ...forecast,
        niv:     Math.round(blend(forecast.niv,     actual.niv)),
        isShort: blend(forecast.niv, actual.niv) < 0,
        wf:      Math.round(blend(forecast.wf,      actual.wf)      * 1000) / 1000,
        sf:      Math.round(blend(forecast.sf,      actual.sf)      * 1000) / 1000,
        sbp:     Math.round(blend(forecast.sbp,     actual.sbp)     * 100)  / 100,
        ssp:     Math.round(blend(forecast.ssp,     actual.ssp)     * 100)  / 100,
        baseRef: Math.round(blend(forecast.baseRef, actual.baseRef) * 100)  / 100,
    };
    return { ...market, forecast: refinedForecast };
}

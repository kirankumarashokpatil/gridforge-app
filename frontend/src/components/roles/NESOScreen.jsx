        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
            ...
            {!isPreGame && readinessEntries.length > 0 && (
                <div style={{
                    ...
                    background: playerReadiness.allReady ? "#071f13" : "#07101f",
                    border: `1px solid ${playerReadiness.allReady ? "#1de98b" : "#4d7a96"}`,
                    borderRadius: 6, minWidth: 90,
                }}>
                    <div style={{ fontSize: 8, fontWeight: 800, letterSpacing: 1, color: playerReadiness.allReady ? "#1de98b" : "#4d7a96" }}>
                        {playerReadiness.allReady ? "✅ ALL READY" : `${playerReadiness.readyCount || 0}/${playerReadiness.totalCount || 0} READY`}
                    </div>
                    <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
                        {readinessEntries.map(([pid, info]) => (
                            <div key={pid}
                                title={`${info.name} (${info.role}) — ${info.ready ? 'Ready' : 'Pending'}`}
                                style={{
                                    width: 7, height: 7, borderRadius: "50%",
                                    background: info.ready ? "#1de98b" : "#2a4a6a",
                                    border: `1px solid ${info.ready ? "#1de98b" : "#4d7a96"}`,
                                }} />
                        ))}
                    </div>
                </div>
    ...
    const left = (
        <div style={{ padding: 8, display: "flex", flexDirection: "column", height: "100%" }}>
            <ForecastPanel
                ...
                forecastUpdateSummary={props.forecastUpdateSummary}
                phase={props.phase}
            />
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <div style={{ ...s.panel, flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                ...
                <div style={{ flex: 1, overflowY: "auto" }}>
                    {/* Show warning if API is not yet connected */}
                    {ready !== true && (
                        <div style={{ textAlign: "center", color: "#f5b222", padding: 40, fontSize: 12, fontWeight: 700 }}>
                            Connecting to server — live bids unavailable. Waiting for players to submit offers.
                        </div>
                    )}

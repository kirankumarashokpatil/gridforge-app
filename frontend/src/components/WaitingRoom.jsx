...
import { normalizePlayer } from "../shared/utils.js";
...
const ACTIVE_HOST_WINDOW_MS = 120000;
...
export default function WaitingRoom({
    ...
    const getAuthoritativeHostId = (playerList) => {
        ...
        const isRecentlyCreated = (createdAt) => {
            ...
            return Number.isFinite(ts) && (nowTs - ts) < ACTIVE_HOST_WINDOW_MS;
        };
        const activePlayersForHost = (playerList || [])
            .filter(p => p?.id)
            .filter(p => {
                const lastSeenTs = p.lastSeen || p.last_seen || 0;
                // lastSeen=0 is treated as active only for newly created rows
                // to avoid resurrecting stale historical players forever.
                if (lastSeenTs === 0) return isRecentlyCreated(p.createdAt || p.created_at);
                return nowTs - lastSeenTs < ACTIVE_HOST_WINDOW_MS;
            })
            .sort((left, right) => {
                const leftTsRaw = left.createdAt ? new Date(left.createdAt).getTime() : 0;
                const rightTsRaw = right.createdAt ? new Date(right.createdAt).getTime() : 0;
        ...
        if (activePlayersForHost.length === 0) return null;

        // Prefer active NESO holder if present; otherwise first active by join order.
        const activeNeso = activePlayersForHost.filter(p => p.role === "NESO");
        if (activeNeso.length > 0) return activeNeso[0].id;
        return activePlayersForHost[0].id;
    };
        ...
        if (api && pid) {
            api.createRoom(room, scenarioId || "BAU")
                ...
                .then(async (registeredPlayer) => {
                    if (registeredPlayer && typeof registeredPlayer === 'object') {
                        mergePlayers([registeredPlayer]);
                        if (registeredPlayer.role === "NESO") {
                            setIsHost(true);
                            setRole("NESO");
                        }
                    }
                                ...
                                <button
                                    onClick={() => {
                                        ...
                                        api.putPlayer(room, pid, {
                                            ...
                                            ready: true,
                                            lastSeen: Date.now(),
                                        }).catch(err => console.error('[WaitingRoom] Claim NESO failed:', err));
                                    }}
                                    style={{
                                        width: "100%",
                                        marginBottom: 10,

...
class Database:
    ...
    async def connect(self):
        ...
        if dsn:
            ...
            self.pool = await asyncpg.create_pool(dsn=dsn, min_size=2, max_size=20)
            ...
            self.pool = await asyncpg.create_pool(
                ...
                database=os.getenv('DB_NAME') or os.getenv('PGDATABASE', 'gridforge'),
                host=os.getenv('DB_HOST') or os.getenv('PGHOST', 'localhost'),
                port=int(os.getenv('DB_PORT') or os.getenv('PGPORT', 5432)),
                min_size=2,
                max_size=20,
            )
            print(f"Connected to PostgreSQL at {os.getenv('DB_HOST') or os.getenv('PGHOST', 'localhost')}")
        await self.init_db()

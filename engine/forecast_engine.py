...
class ForecastEngine:
    def __init__(self, sp_per_day: int = 48):
    ...
    def _model_demand(self, noise_mult: float) -> list[float]:
        ...
        for sp in range(self.sp_per_day):
            ...
            if 0 <= hour <= 5:
                val *= 0.7
            noise = 1 + (random.random() - 0.5) * self.params["noise_level"] * noise_mult
            curve.append(round(val * noise, 1))
        return curve

    def _model_wind(self, noise_mult: float) -> list[float]:
        curve = []
        current_yield = 0.4 + random.random() * 0.2
        for sp in range(self.sp_per_day):
            current_yield += (random.random() - 0.5) * 0.1 * noise_mult
    ...
    def _model_solar(self, noise_mult: float) -> list[float]:
        ...
        for sp in range(self.sp_per_day):
            ...
            if 6 < hour < 18:
                dist = abs(12 - hour)
                val = max(0, 1 - dist / 6)
            noise = 1 + (random.random() - 0.5) * self.params["noise_level"] * 2 * noise_mult

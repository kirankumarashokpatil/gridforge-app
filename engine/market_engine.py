...
def _wind_speed_for_sp(sp: int) -> float:
    base = 6 + 3 * math.sin(((sp - 1) / 48) * math.pi * 2)
    w_rng = _rng(sp * 4211 + 17)
    noise = (w_rng() - 0.5) * 2
    return max(0.0, base + noise)


def _forecast_wind_speed(true_speed: float, sp: int) -> float:
    err_scale = 0.12 + 0.08 * _rng(sp * 3127 + 9)()
    sign = -1 if _rng(sp * 531 + 3)() < 0.5 else 1
...
def _solar_irradiance_for_sp(sp: int) -> float:
    hr = ((sp - 1) / 2) % 24
    if hr < 6 or hr > 18:
        return 0.0
    base = math.sin(((hr - 6) / 12) * math.pi)
    s_rng = _rng(sp * 715 + 23)
    noise = (s_rng() - 0.5) * 0.3
    return clamp(base + noise, 0.0, 1.0)


def _forecast_solar_irradiance(true_irr: float, sp: int) -> float:
    err_scale = 0.07 + 0.03 * _rng(sp * 811 + 5)()
    sign = -1 if _rng(sp * 929 + 2)() < 0.5 else 1
...
def market_for_sp(
    ...
    true_wind_speed = _wind_speed_for_sp(sp)
    forecast_wind_spd = _forecast_wind_speed(true_wind_speed, sp)
    expected_wind = _wind_fraction_from_speed(forecast_wind_spd) * sc["windMod"]

    true_irr = _solar_irradiance_for_sp(sp)
    forecast_irr = _forecast_solar_irradiance(true_irr, sp)
...
def compute_forecasts(
    ...
    for offset in range(1, max_offsets + 1):
        ...
        state = market_for_sp(sp, scenario_id, [], published_forecast)
        f = state["forecast"]
        fcasts.append({
            **f,
            "sp": f["sp"],
